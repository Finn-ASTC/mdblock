...
### 交付后复查修掉的三条（2026-09-18，控制方）

一次「这东西真能拿出去用吗」的复查跑出三条问题，都已修并补了回归测试
（164 个单元测试 + 29 条验收判据全绿）：

1. **内置主题按 cwd 解析 → 出不了仓库**（契约修订 A-3）。改为由 `src/theme.ts` 自身
   位置推出 `<包根>/themes`。验收新增 X-6 从 `dist/` 里跑真实 CLI 验证。
2. **批量同名 basename 静默覆盖**（A-4）。改为用法错误（退出码 2，列出冲突的两个输入）。
   验收新增 X-5，且断言失败**原因**而不只是退出码。
3. **没有用户文档**。补 `README.md`：CLI 参数表、主题自定义、CSS 变量 API、
   三种 `--css` 模式的取舍、已知限制。

这一轮的教训与前面一致：**160 个测试 + 26 条验收判据全绿，仍然不代表「能交付」** ——
前三者关心契约内行为，而「装到别处还能不能跑」这类问题只有真的换个目录跑一次才会暴露。
复查本身也修正了我原先写在报告里的判断：我曾把「themes/ 按 cwd 解析」列为可接受的已知边界，
实际上它是核心功能的失效点。

### 演示时当场发现的第四条：后代文字色靠继承，被宿主一行 `p{color}` 改掉（A-5）

把产物贴进一个假装的真实博客（宿主有 `.prose p { color: #6b7280 }`）截图时，
**两个主题的正文全变成宿主的灰色**。根因：文字色只声明在根元素上，段落靠继承拿 ——
而继承永远输给任何直接命中 `p` 的规则，宿主那条特异性低到 (0,0,1) 也一样赢。
修法见契约修订 A-5（两级 `color: inherit`），并补了三条浏览器判据与一层新的敌意宿主规则。

**这暴露了验收判据的又一个盲区**：我的 L1 判据测了段落的 `margin`、链接的 `color`、
表格的 `border`……**唯独没测段落的 `color`** —— 因为写判据时我脑子里的假设是
「颜色由根元素负责」，而那个假设本身就是缺陷。**判据里没有的东西，等于没测。**
